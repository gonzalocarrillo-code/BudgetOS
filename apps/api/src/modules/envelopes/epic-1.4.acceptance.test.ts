...
afterAll(async () => {
  ...
  if (golden?.created) {
    ...
    await owner.$executeRawUnsafe(`DELETE FROM ingest_run WHERE source_id IN (SELECT id FROM data_source WHERE workspace_id = $1::uuid)`, ws);
    await owner.$executeRawUnsafe(`DELETE FROM data_source WHERE workspace_id = $1::uuid`, ws);
