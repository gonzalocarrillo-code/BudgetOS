...
run(join(E2E_DIR, "../node_modules/.bin/tsx"), ["src/main.ts"], join(E2E_DIR, "../../api"), { ...dbEnv(), PORT: String(PORTS.api), AUTH_AUDIENCE: PROJECT, AUTH_ISSUER: ISSUER, AUTH_JWKS_URL: `http://127.0.0.1:${PORTS.jwks}/jwks`, CLOSURE_SINK: "memory" });
run(join(E2E_DIR, "../../workers/node_modules/.bin/tsx"), ["src/local-runner.ts"], join(E2E_DIR, "../../workers"), { ...dbEnv(), PORT: String(PORTS.worker) });
