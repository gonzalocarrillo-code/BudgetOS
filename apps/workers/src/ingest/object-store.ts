...
/**
 * Where uploads and rejected-rows reports live (ADR-011). GCS in deployed environments and against
 * the emulator locally (GCS_EMULATOR_HOST); memory for the golden seed and unit tests.
 */
export interface ObjectStore {
  read(uri: string): Readable;
  write(uri: string, body: string, contentType: string): Promise<void>;
...
export class MemoryObjectStore implements ObjectStore {
  ...
  async write(uri: string, body: string, contentType: string): Promise<void> {
...
/** Bucket for CSV uploads and rejected-rows reports. */
